...
################################################################################
  # Print starting time
  bs <- begin.script(paste('[', PROJECT, '] 02_explore.R', sep = ''))
  ...
  # Summary statistics
  txt.file <- paste(TEMPDIR, 'variable_summary.txt', sep = '')
  sink(txt.file)
  ...
  for (i in 1:ncol(np.train)) {
    if (class(np.train[, i]) != 'character') {
      ...
      legend('topright', as.character(1:5),
             col = c('red', 'orange', 'blue', 'darkgreen', 'black'),
             lwd = 2)
      dev.off()
